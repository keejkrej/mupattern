...
- **mupattern-py** (pure Python CLI, reference): Top-level inference: `convert`, `crop`, `movie`, `kill`. Python-only: `plot kill`, `train kill`, `dataset kill`. For ONNX export (for mupattern-desktop), use `uv run optimum-cli export onnx` — see models/README.md. `kill` runs predict then clean (monotonicity) in one pipeline. Prod code lives in mupattern-desktop (Rust binary + ONNX). Uses `nd2` (nd2-py) for ND2; `common.nd2_utils.read_frame_2d(f, p, t, c, z)` for 2D Y×X frames.
- **mupattern-rs** (Rust CLI): `convert` (ND2→TIFF), `crop`, `kill`, `movie`. Uses `nd2-rs`, `zarrs`, and `ort` (ONNX). Build: `cargo build`; run: `cargo run -p mupattern-rs -- kill --input /path/to/crops.zarr --pos 150 --model models/mupattern-resnet18 --output predictions.csv`. ND2 access: `sizes()`, `read_frame_2d(p,t,c,z)`.
...
- **crops.zarr** layout (Zarr v3 only): `pos/{pos:03d}/crop/{crop_id}` arrays (T, C, Z, H, W); optional `pos/{pos:03d}/background` (T, C, Z) per-pixel. Kill predictions CSV: `t,crop,label`.
- JS app (web): `mupattern-web` — lite web app (landing, register, see), deployed on Firebase; run with `bun run dev` from that directory
- JS app (desktop): `mupattern-desktop` — Electron workspace-first app; Tasks (convert, crop, movie, kill) with Clean completed; run with `bun run dev` from that directory

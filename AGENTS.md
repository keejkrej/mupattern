...
- **mupattern-rs** (Rust CLI): `convert` (ND2→TIFF), `crop`, `expression`, `kill`, `movie`. Uses `nd2-rs`, `zarrs`, `ort` (ONNX). Build: `cargo build`; run: `cargo run -p mupattern-rs -- kill --input /path/to/crops.zarr --pos 150 --model models/mupattern-resnet18 --output predictions.csv`. ND2 access: `sizes()`, `read_frame_2d(p,t,c,z)`.

...
export default function WorkspaceDashboard() {
  ...
  const [pathExistsByWorkspaceId, setPathExistsByWorkspaceId] = useState<Record<string, boolean>>(
    {},
  );
  ...
  const handleAddWorkspace = useCallback(async () => {
    ...
    try {
      ...
      const workspace: Workspace = {
        ...
        workspaceTags: [],
        channels: result.channels.length > 0 ? result.channels : [0],
        times: result.times.length > 0 ? result.times : [0],
        zSlices: result.zSlices.length > 0 ? result.zSlices : [0],
        selectedChannel: result.channels[0] ?? 0,
        selectedTime: result.times[0] ?? 0,
        selectedZ: result.zSlices[0] ?? 0,
        currentIndex: 0,
      };

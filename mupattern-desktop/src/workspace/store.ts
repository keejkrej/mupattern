...
function getWorkspaceStateApi() {
  if (typeof window !== "undefined" && window.mupatternDesktop?.workspaceState) {
    return window.mupatternDesktop.workspaceState;
  }
  return {
...
function normalizeWorkspace(workspace: Workspace): Workspace {
  ...
  const positionsFromState = Array.isArray(workspace.positions)
    ? workspace.positions
        .map((value) => normalizePositionValue(value))
  ...
  const positionFilterLabelsRaw = (() => {
    ...
    if (Array.isArray(next))
      return next.map((v) => String(v ?? "").trim()).filter((v) => v.length > 0);
    return [];
  })();
  const tagLabelSet = new Set(posTags.map((tag) => tag.label));
  const positionFilterLabels = [...new Set(positionFilterLabelsRaw)].filter((label) =>
...
async function hydrateWorkspaceStoreFromDisk(): Promise<void> {
  try {
    ...
    if (saved && isWorkspaceStoreState(saved)) {
      ...
      const validActiveId =
        saved.activeId && workspaces.some((w) => w.id === saved.activeId) ? saved.activeId : null;
...
// --- Filename builder (convert task output format) ---
...
export function updateWorkspaceScan(
  workspaceId: string,
  scan: { positions: number[]; channels: number[]; times: number[]; zSlices: number[] },
) {
  workspaceStore.setState((s) => ({
    ...s,
    workspaces: s.workspaces.map((w) =>
      w.id === workspaceId
        ? normalizeWorkspace({
            ...
            channels: scan.channels.length > 0 ? scan.channels : [0],
            times: scan.times.length > 0 ? scan.times : [0],
            zSlices: scan.zSlices.length > 0 ? scan.zSlices : [0],
          })
        : w,
    ),
  }));
...
export function setSelectedTime(workspaceId: string, value: number) {
  workspaceStore.setState((s) => ({
    ...s,
    workspaces: s.workspaces.map((w) => (w.id === workspaceId ? { ...w, selectedTime: value } : w)),
  }));
}
...
export function setSelectedZ(workspaceId: string, value: number) {
  workspaceStore.setState((s) => ({
    ...s,
    workspaces: s.workspaces.map((w) => (w.id === workspaceId ? { ...w, selectedZ: value } : w)),
  }));
}
...
export function addPositionTag(
  workspaceId: string,
  label: string,
  startIndex: number,
  endIndex: number,
 ...
 */
export function setPositionTagsFromDict(workspaceId: string, dict: Record<string, string>): void {
  ...
  const visible = allIndices.filter((index) =>
    uniqueFilterLabels.every((label) =>
      workspace.posTags.some(
        (tag) => tag.label === label && index >= tag.startIndex && index <= tag.endIndex,
      ),
    ),
  );
  return visible;
...
export async function readPositionImage(posNum: number): Promise<LoadedWorkspaceImage | null> {
  ...
  try {
    const result = await window.mupatternDesktop.workspace.readPositionImage({
...
export async function saveWorkspaceBboxCsv(
  ...
  try {
    return await window.mupatternDesktop.workspace.saveBboxCsv({
      workspacePath: ws.rootPath,
      pos,
      csv,
    });

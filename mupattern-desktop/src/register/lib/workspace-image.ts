...
export async function reloadActiveWorkspaceImage(): Promise<
  { ok: true; source: ImageSource } | { ok: false; error: string }
> {
  ...
  if (!loaded) {
    return {
      ok: false,
      error: "Could not read selected file. Try a different position/channel/time/z.",
    };
  }
